...
has_field 'search_history'
  => ( type                  => 'Checkbox',
       ...
       wrapper_class         => [ 'offset-md-2', 'form-check', ],
...
has_field 'search_filter'
  => ( type                  => 'TextArea',
       label                 => 'Search Filter',
       label_class           => [ 'col', 'text-right', 'font-weight-bold', 'required' ],
       wrapper_class         => [ 'row', 'searchaccount', ],
       element_wrapper_class => [ 'input-sm', 'col-10', ],
...
has_field 'search_results'
  => ( label                 => 'Search Results',
       label_class           => [ 'col', 'text-right', 'font-weight-bold', ],
       element_wrapper_class => [ 'input-sm', 'col-10', ],
       element_attr          => { placeholder => '50',
				  title => q{A sizelimit that restricts the maximum number of entries to be returned as a result of the search. A value of 0, means that no restriction is requested.}, },      wrapper_class         => [ 'row', ],
);

has_field 'search_scope'
  => ( type                  => 'Select',
       label                 => 'Search Scope',
       label_class           => [ 'col', 'text-right', 'font-weight-bold', ],
...
sub build_render_list {[ qw( search_history
			     base_dn
			     reqType
			     reqAuthzID
			     reqDn
...
sub validate {
  ...
  my $err_history = 'Filter is mandatory, you have entered no any!';
  if ( $self->field('search_history')->value eq '1' &&
    ...
    $self->field('reqAuthzID')->add_error($err_history);
    $self->field('reqDn')->add_error($err_history);
    $self->field('reqEnd')->add_error($err_history);
    $self->field('reqMessage')->add_error($err_history);
    $self->field('reqMod')->add_error($err_history);
    $self->field('reqOld')->add_error($err_history);
    $self->field('reqResult')->add_error($err_history);
    $self->field('reqStart')->add_error($err_history);
    $self->field('reqType')->add_error($err_history);
    #$self->add_form_error($err_history);
  }
}
