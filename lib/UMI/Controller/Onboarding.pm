...
sub index :Path :Args(0) {
  ...
  if ( defined $c->user_exists && $c->user_exists == 1 ) {
    ...
    ### --- check for existent parts start ---------------------

    my %check_acc = ( vault  => q{authorizedService=web@vault.norse.digital},
		      gitlab => q{authorizedService=gitlab@gitlab.norse.digital},
		      docker => q{authorizedService=web@tools.norse.co},	  );

    foreach ( sort (keys %check_acc) ) {
	...
	if $mesg->code != 0 && $mesg->code != 32;
      push @{$key->{html}->{warning}}, sprintf("<kbd>%s</kbd> account exists", uc($_))
	if $mesg->count > 0;
    }
    ...
    foreach ( $mesg->entries ) {
      $tmp++ if $_->get_value( 'sshPublicKey' ) =~ /.* $self->{a}->{re}->{sshpubkey}->{comment} .*/;
    }
    push @{$key->{html}->{warning}}, "<kbd>SSH</kbd> key exists"
      if $tmp;

    $mesg =
      $ldap_crud->search( { base   => $ldap_crud->cfg->{base}->{pgp},
			    filter => sprintf('pgpUserID=*%s %s*%s*',
					      $c->user->givenname,
					      $c->user->sn,
					      $self->{a}->{re}->{gpgpubkey}->{comment}),
			    attrs  => ['pgpUserID'], } );
    push @{$key->{html}->{error}}, $ldap_crud->err($mesg)->{caller} . $ldap_crud->err($mesg)->{html}
      if $mesg->code ne '0';
    ...
    $tmp = 0;
    foreach ( @{[$mesg->entries]} ) {
      $tmp++ if $_->get_value( 'pgpUserId' ) =~ /.*$self->{a}->{re}->{gpgpubkey}->{comment} .*/;
    }
    push @{$key->{html}->{warning}}, "<kbd>GPG</kbd> key exists"
      if $tmp;

    push @{$key->{html}->{warning}}, '<hr>', '<div class="alert alert-warning" role="alert">
  <i class="fas fa-exclamation-triangle"></i> to change password for accounts go to
<a href="/user_prefs" class="btn btn-link"> profile &rarr; Services</a> (upper right corner dropbox), to re-generate key/s use items on the left sidebar <i class="fas fa-lg fa-fw fa-toolbox"></i> TOOLS &rarr;<a href="/toolsshkeygen" class="btn btn-link">SSH keygen</a> / <a href="/toolgpgkeygen" class="btn btn-link">GPG keygen</a></div>

<div class="alert alert-danger" role="alert">
  <i class="fas fa-exclamation-triangle"></i> SSH and GPG keys will be re-generated, old ones will be deleted! You will need to provide admin with pub part of the newly generated keys!
</div>

'
      if defined $key->{html}->{warning};

    ### --- check for existent parts stop ----------------------

    $c->stash( template => 'tool/onboarding.tt',
	       final_message => $key->{html},
	       form     => $self->form );
